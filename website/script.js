...
window.addEventListener("load", function(){ // anonymous clojure but code will only be executed after website was loaded completely
	...
	var Series = Object.freeze( // freezing prevents editing of object
		[
			...
			// E192 - currently disabled for performance reasons but all entries are right
	...
	var ActiveSeries = undefined; // select which series is in use for the calculation next
	Series.forEach( function(e) { // create selection button in pre-defined HTML list for each series
		var new_node = document.getElementsByClassName("selection_row")[0].appendChild(Object.assign(document.createElement("li"), { // append new node (list-entry) to list
			innerText: `E${e.length}`, // assign text to node HTML content
			onclick: function() { // click event listener: set active E-Series and css classes for selection objects. The wrapper provides a separate closure
				var series_entry = e;
