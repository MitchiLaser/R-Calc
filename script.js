...
window.addEventListener("load", function(){ // anonymous clojure but code will only be executed after website was loaded completely
	...
	var Series = Object.freeze( // freezing prevents editing of object
		[
			...
			// E192 - currently disabled for performance reasons
	...
	// End E-Series definition
	
	var ActiveSeries = undefined; // select which series is in use for the calculation

	Series.forEach( function(e) {
		var new_node = document.getElementsByClassName("selection_row")[0].appendChild(Object.assign(document.createElement("li"), {
			innerText: `E${e.length}`, // assign text to node HTML content
			onclick: function() { // click event listener: set active E-Series and css classes for selection objects
				var series_entry = e;
